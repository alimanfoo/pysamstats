...
def generate_fixtures():
    ...
    # simple stats
    dat1 = {q: getattr(pysamstats, q)(Samfile(bampath))
          for q in ["load_coverage", "load_baseq", "load_mapq"]}

    dat2 = {q: getattr(pysamstats, q)(Samfile(bampath))
            for q in ["load_coverage", "load_baseq", "load_mapq"]}

    assert not isfile(archive), "Attempting to regenerate static test archive."
    np.savez_compressed(archive, **dat1)
...
def test_against_fixtures():
    ...
    for q in ["load_coverage", "load_baseq", "load_mapq"]:
        ...
        # for alpha, beta in zip(testset[q], x):
        ...
        for key in testset[q].dtype.fields.keys():
            try:
                ...
                ix = testset[q][key] != x[key]
                print(ix.sum(), "/", ix.size, "errors", key)
                qq = np.compress(ix, testset[q], axis=0)
                zz = np.compress(ix, x, axis=0)
                for a, b in zip(qq, zz):
                    print(a["pos"])
                    print(a["rms_mapq"])
                    print(b["rms_mapq"])
